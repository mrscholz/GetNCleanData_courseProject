---
title: "Read Me file for the Get and Clean Data course project: run_analysis.R"
output: 
  pdf_document: 
    highlight: espresso
---
...
The script *run_analysis* is designed to join, clean and summarize a machine learning dataset from
[http://archive.ics.uci.edu/ml/datasets/Human+Activity+Recognition+Using+Smartphones]

It assumes that the data is organized in subfolders containing test and train datasets.
It takes the path to the unzipped data as an argument, but by default it assumes that the current working directory contains the unzipped folder "UCI HAR Dataset".
...
### I transform data.tables back to data.frames; this is not absolutely necessary but there are some issues with naming of columns and merging the data, which I was just to lazy to figure out
```
X <- data.frame(X)
y <- data.frame(y)
```
...
### I want both data.frames (tidy_group and joinedData) returned by the script so I store them in a list
```
